/* Função serverless: CLIENTES ENCERRADOS (contato encerrado pela equipe).
   Netlify Blobs. GET -> {encerrados}. POST {acao:'add'|'remove', cod, ..., senha}.
   PERMANENTE: nada expira — fica tudo rastreado e catalogado.
   Motivos: livres (Em débito / Sem interesse / Judicial / qualquer novo digitado).
   Segredo (senha do time CRM) injetado no deploy (secret.mjs). */
import { getStore } from "@netlify/blobs";
import { SECRET } from "./secret.mjs";

export default async (req) => {
  const store = getStore("crm-encerrados");
  const load = async () =>
  ...
  if (req.method === "GET")
    return Response.json({ encerrados: await load() }, { headers: cors });
  ...
  if (req.method === "POST") {
    ...
    return Response.json({ ok: true, encerrados: await load() }, { headers: cors });
  }
  return new Response("metodo nao permitido", { status: 405, headers: cors });
};

export const config = { path: "/api/crm-encerrados" };
