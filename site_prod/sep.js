...
(function () {
  ...
  let selByView = { separar: '', urgente: '', avisar: '', travados: '', hist: '', apagados: '' }; // categoria selecionada por aba
  ...
  async function post(payload) {
    try {
      if (useSupa()) {
        ...
        else if (a === 'receber') { const _hist = ehHistotecnica((marks[k] && marks[k].cat) || payload.cat); await window.SUPA.updateMark(k, { estado: _hist ? 'suficiente' : 'recebido', por_receb: payload.por || 'equipe', ts_receb: Date.now() }); } // Dudu 26/08: histotécnica → recebi já finaliza (o pote/PL já confirma amostra)
  ...
  // 🔒 TRAVAR o exame — sai do fluxo e vai pra aba Travados (motivo em texto livre no campo de observação)
  ...
  /* ================= RENDER ================= */
  function header() {
    const sepN = naFilaHoje().length, urgN = ultChamadaPendN(), aviN = aAvisar().length, ncN = naoClass().length, travN = aTravar().length;
    ...
    return `<div class="sephead">
      <div class="septabs">
        ...
        <div class="septab trava ${travN ? 'travapulse' : ''} ${view === 'travados' ? 'on' : ''}" data-v="travados">🔒 Travados <span class="c">${travN}</span></div>
  ...
  function rowSeparar(it) {
    ...
    let b3;
    if (ehHistotecnica(it.cat)) b3 = '';   // Dudu 26/08: histotécnica NÃO tem passo "tem amostra?" — o pote/PL já confirma; "Receber" já finaliza (vai pro histórico)
    else if (received && travadoHisto(it)) { const why = 'Histotécnica: só David e Eduardo confirmam a amostra'; b3 = `<button class="sepbtn insuf off" data-act="insufoff" data-k="${k}" data-why="${esc2(why)}" title="${esc2(why)}"><span>🚫 Tem amostra?</span><small>🔒 David/Eduardo</small></button>`; }
    ...
    // 🔒 TRAVAR: manda o exame pra aba Travados (financeiro/cadastro/problema de amostra…), motivo em texto livre
    const btrava = `<button class="sepbtn trava" data-act="travar" data-k="${k}" title="travar este exame (financeiro, cadastro, problema de amostra…)">🔒 Travar</button>`;
    return `<div class="seprow ${received ? 'confirmrow' : ''}">${head}<div class="right2">${badge}${b1}<span class="steparrow">→</span>${b2}${b3}${btrava}${undo}</div></div>`;
  }
  ...
  /* ---- 🔒 TRAVADOS: exames travados (financeiro/cadastro/problema de amostra) — motivo em texto livre (obs) ---- */
  const vezesTxt = m => { const n = Number(m.corte) || 1; return n > 1 ? ` · 🔁 travado ${n}×` : ''; };
  ...
  function viewTravados() {
    ...
    if (!items.length && !hist.length) return `<div class="sepwait">✓ Nenhum exame travado. 👍</div>`;
    let out = '';
    if (items.length) {
      const byCat = {}; items.forEach(m => { const c = m.cat || '—'; (byCat[c] = byCat[c] || []).push(m); });
      const cats = orderedCats(byCat);
      let sel = selByView.travados; if (!sel || !byCat[sel]) sel = selByView.travados = cats[0];
      const strip = topicStrip(byCat, 'travados', sel, null);
      const arr = byCat[sel].slice().sort((a, b) => (Number(a.ts_sep) || 0) - (Number(b.ts_sep) || 0));
      const bar = `<div class="andonbar trava"><span class="ico">🔒</span><span class="ttl">${items.length} EXAME${items.length > 1 ? 'S' : ''} TRAVADO${items.length > 1 ? 'S' : ''} · RESOLVA O MOTIVO (financeiro, cadastro, amostra) E DESTRAVE!</span></div>`;
      out += bar + strip + `<div class="sepcat andon travamax"><div class="h"><span>🔒 ${esc2(sel)}</span><span class="cnt">${arr.length} travado(s)</span></div>${arr.map(rowTravar).join('')}</div>`;
    } else {
      out += `<div class="sepnote" style="background:rgba(22,163,74,.1);border:1px solid rgba(22,163,74,.4);color:#15803d;padding:10px 14px;border-radius:10px;margin-bottom:10px">✓ Nenhum exame travado agora. Veja o histórico abaixo.</div>`;
    }
    // 📋 HISTÓRICO dos liberados (o que Wal pediu: dias travado, quem travou, quantas vezes, quem liberou)
    if (hist.length) {
      out += `<div class="sepcat" style="margin-top:12px"><div class="h"><span>📋 Histórico de travados (liberados)</span><span class="cnt">${hist.length}</span></div>${hist.slice(0, 100).map(rowTravarHist).join('')}</div>`;
    }
    return out;
  }
  ...
  function viewPlacar() {
    ...
    universo().forEach(u => {
      ...
      const m = marks[k];
      if (m && (m.estado === 'insuficiente' || m.estado === 'insuficiente_avisado' || m.estado === 'travado' || m.estado === 'destravado')) return; // insuficiente/travado = NEUTRO (fora do placar)
  ...
  const LEGENDS = {
    ...
    travados: '🔒 Exames TRAVADOS por algum motivo (financeiro, cadastro, problema de amostra…). O motivo fica no campo de observação. Resolva e clique "✓ Destravar" pra voltar ao fluxo.',
  ...
  function render() {
    ...
    else if (view === 'travados') body = viewTravados();
  ...
  function wire(el) {
    ...
    el.querySelectorAll('[data-act]').forEach(b => b.onclick = () => {
      ...
      else if (act === 'travar') { const it = itens().find(x => chaveOf(x) === k); if (it) doTravar(it); }
      else if (act === 'destravar') step('destravar', k, 'Destravar este exame? Volta pro fluxo de separar.');
